...
(function($) {
	$.fn.kinMaxShow = function(user_options) {
		...
		k.setLayout = function() {
			...
			//显示内容
			$('.' + k.prename + 'image_item:gt(0)', k.self).css('z-index', -3).css({
				opacity: 0
			});
		...
		k.setCSS = function() {
			...
			if (options.button.normal.display != 'none') {
				cssCode += k.selector + ' .' + k.prename + 'button{' + k.fn.objToCss(options.button.normal, ['top', 'right', 'bottom', 'left'], true) + ';position:absolute;right:250px;list-style:none;z-index:-1;border-radius:10px;}';
				cssCode += k.selector + ' .' + k.prename + 'button li{' + k.fn.objToCss(options.button.normal, ['top', 'right', 'bottom', 'left']) + ';cursor:pointer;-webkit-text-size-adjust:none;border-radius:6px;}';
				cssCode += k.selector + ' .' + k.prename + 'button li.focus{' + k.fn.objToCss(options.button.focus, ['top', 'right', 'bottom', 'left']) + ';cursor:default;border-radius:6px;}';
			}
			cssCode += '</style>';
		...
		k.setOpacity = function() {
			...
			//设置上一个显示的z-index为-3
			$('.' + k.prename + 'image_item', k.self).css('z-index', -3);
			//设置当前显示的z-index为-2
